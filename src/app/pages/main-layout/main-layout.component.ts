...
@Component({
  ...
  standalone: true,
  imports: [CommonModule, FormsModule, SendTopicListComponent, SubscribeTopicsComponent, MessageEditorComponent, ConnectionManagerComponent],
...
export class MainLayoutComponent implements OnInit, OnDestroy {
  ...
  onReconnectRequested(tabId: number): void {
    ...
    console.log(`Reconnecting to ${tab.label}...`);
    ToastUtil.info('Reconnecting', `Attempting to reconnect to ${tab.label}...`);

    this.connectToMqtt(tabId, tab.config)
      .then(() => {
        console.log(`Reconnected to ${tab.label}`);
        ToastUtil.success('Connected', `Successfully reconnected to ${tab.label}`);
        this.debounceSave();
      })
      .catch(error => {
        console.error(`Failed to reconnect to ${tab.label}:`, error);
        ToastUtil.error('Connection Failed', error.message || 'Failed to reconnect');
        this.debounceSave();
      });
  }
